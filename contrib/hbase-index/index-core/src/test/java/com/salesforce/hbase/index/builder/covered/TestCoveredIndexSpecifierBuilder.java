...
public class TestCoveredIndexSpecifierBuilder {
  ...
  public void testSimpleSerialziationDeserialization() throws Exception {
    ...
    assertEquals("Didn't deserialize expected column in first group", col1, group.get(0));
    assertEquals("Didn't deserialize expected column in first group", col2, group.get(1));
    group = columns.get(1);
    assertEquals("Didn't deserialize expected column in second group", col3, group.get(0));
  }
}